		...
		if (IS_POST) {
			...
			if ($p) {
				...
				foreach ($site as $t) {
                    ...
                    if ($module) {
                        foreach ($module as $m) {
                            if (!$this->db->query("SHOW TABLES LIKE '%".$this->db->dbprefix($t['id'].'_'.$t['dirname'].'_index')."%'")->row_array()) {
                                continue;
                            }
                            // 替换主表URL
                            $this->db->query('update '.$this->db->dbprefix($t['id'].'_'.$t['dirname']).' set url=REPLACE(url, "http://'.$t['domain'].'", "http://'.$domain.'")');
                            // 更新附表内容URL
                            for ($i = 0; $i<200; $i++) {
                                $table = $this->db->dbprefix($t['id'].'_'.$t['dirname'].'_data_'.$i);
                                if (!$this->db->query("SHOW TABLES LIKE '%".$table."%'")->row_array()) {
                                    continue;
                                }
                                $this->db->query('update '.$table.' set content=REPLACE(content, "http://'.$t['domain'].'", "http://'.$domain.'")');
                            }
                        }
                    }
				}
